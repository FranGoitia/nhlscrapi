...
def is_close(score, play):
    close = False
    scr = 0
    if len(score.teams):
        t = score.teams[0]
        scr = score.total[t]
    if len(score.teams) > 1:
        t = score.teams[1]
        scr -= score.total[t]
...
class BlockCt(ShotEventTallyBase):
    """
    Tallies blocks for each team. Increments if

      * the play event inherits from :py:class:`.Block`
...
class EvenStBlockCt(ShotEventTallyBase):
    """
    Tallies blocks for each team. Increments if
...
class EvenStClBlockCt(ShotEventTallyBase):
    """
    Tallies blocks for each team. Increments if
    ...
    def update(self, play):
        ...
        super(EvenStClBlockCt, self).update(play)


class MissCt(ShotEventTallyBase):
    """
    Tallies misses for each team. Increments if

      * the play event inherits from :py:class:`.Miss`
      * at even strength
...
class EvenStMissCt(ShotEventTallyBase):
    """
    Tallies misses for each team. Increments if
...
class EvenStClMissCt(ShotEventTallyBase):
    """
    Tallies misses for each team. Increments if
    ...
    def update(self, play):
        self.score.update(play)
        super(EvenStClMissCt, self).update(play)
...
class EvenStClShotCt(ShotEventTallyBase):
    """
    Tallies even strength shots on goal for each team. Increments if
...
class EvenStClShotAttCt(ShotEventTallyBase):
    """
    Tallies even strength shots on goal for each team. Increments if
    ...
    def update(self, play):
        self.score.update(play)
        super(EvenStClShotAttCt, self).update(play)
...
class Fenwick(ShotEventTallyBase):
    ...
    def share(self):
        ...
        """
        total_fen_shots = sum(self.total.values())
        return {k: v/float(total_fen_shots) for k, v in self.total.items()}
